# Roadmap

- [x] Ground product facts with Open Food Facts and trusted regional search fallback
- [x] Split and validate AI analysis stages
- [x] Add streaming progress and clear error handling
- [x] Add signed-in rate limits and 30-day cache hygiene
- [x] Lazy-load reports and compress uploads
- [x] Deploy backend functions and run focused tests
- [x] Verify desktop and mobile layout
- [x] Run one signed-in end-to-end AI analysis (verified on mobile viewport)
- [x] Send unsigned users to the sign-in screen instead of an error toast
- [x] Add EatSmart icon, favicon, and branded splash screen
- [x] Prevent mobile regulatory details from being clipped
- [x] Correct regulatory evidence semantics and invalidate stale cached reports
- [x] Derive ingredient purity deterministically from visible classifications
- [ ] Ground nutrition facts and serving sizes, then use them in health advice and value scoring
- [ ] Add a signed-in dashboard for frequently searched products and score history
- [ ] Build normalized Open Food Facts and USDA product records keyed by brand, variant, and region
- [ ] Run a signed-in end-to-end product search and verify sources and loading stages
